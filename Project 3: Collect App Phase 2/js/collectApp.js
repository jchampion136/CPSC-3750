    ...
    .then(result => {
      ...
      for (let i = 0; i < maxResults; i++) {
        ...
        cardDiv.innerHTML = `
          ...
          <button onclick="addToFavorites('${card.id}')">Add to Favorites</button>
        `;
...
//Adds a card to the user's favorites
function addToFavorites(cardID) {
  //Sends a POST request to Add card to favorites
  fetch("favorite.php", {
    method: "POST",
    headers: {
      "Content-Type": "application/x-www-form-urlencoded"
    },
    body: "item_id=" + encodeURIComponent(cardID)
  })
  ...
  .catch(error => {
    ...
    alert("An error occurred while adding the card to favorites.");
  });
}
